...
static int drawtitle (const char *text, unsigned long col[ColLast], Bool highlight, Monitor *m, Bool first);
static int drawtags (Monitor *m);
static int drawstatus (unsigned long col[ColLast]);
...
void focusonclick(const Arg *arg);
/* variables */
...
cleanup(void)
{
	...
	for (i = ColBorder; i < ColLast; i++)
	{
		...
		XftColorFree(dpy, DefaultVisual(dpy, screen), DefaultColormap(dpy, screen), dc.xft.urg + i);
	}
...
clientmessage(XEvent *e)
{
	...
	if (cme->message_type == netatom[NetStartupID] ||
	        cme->message_type == netatom[NetStartupInfo] ||
...
drawtags (Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++)
	{
		...
		if (!pango_parse_markup (tags[i], -1, 0, &attrList, &buf, NULL, &error))
		{
			if (error);
			g_error_free (error);
			buf = g_markup_escape_text (tags[i], -1);
		}
		pango_layout_set_attributes(dc.font.layout, attrList);
		pango_layout_set_text (dc.font.layout, buf, -1);
		pango_layout_set_single_paragraph_mode (dc.font.layout, TRUE);
	...
	XSetForeground(dpy, dc.gc, dc.norm[ColBG]);
	if (!pango_parse_markup (m->ltsymbol, -1, 0, &attrList, &buf, NULL, &error))
	{
		if (error)
			g_error_free (error);
		buf = g_markup_escape_text (m->ltsymbol, -1);
	}
	pango_layout_set_attributes(dc.font.layout, attrList);
	pango_layout_set_text (dc.font.layout, buf, -1);
	pango_layout_set_single_paragraph_mode (dc.font.layout, TRUE);
	...
	pango_layout_set_width (dc.font.layout, -1);
	pango_layout_get_extents(dc.font.layout, 0, &r);
	maxwidth = 10 + (int) round(r.width / PANGO_SCALE);
...
int
drawstatus (unsigned long col[ColLast])
{
	...
	pango_xft_render_layout(dc.xft.drawable, dc.xft.norm + ColFG,
	                        dc.font.layout, x * PANGO_SCALE,
	                        y * PANGO_SCALE);
	free (buf);
	pango_layout_set_attributes(dc.font.layout, NULL);
	return bargap + (int) round (r.width / PANGO_SCALE);
}
...
{
	//long data[] = { TAGMASK };
	int desks = sizeof (tags) / sizeof (tags[0]);
	XChangeProperty(dpy, root, netatom[NetNumberOfDesktops], XA_CARDINAL, 32,
	                PropModeReplace, (unsigned char *)&desks, 1);
}
...
setup(void)
{
	...
	netatom[NetStartupID] = XInternAtom(dpy, "_NET_STARTUP_ID", False);
	netatom[NetStartupInfo] = XInternAtom(dpy, "_NET_STARTUP_INFO", False);
	netatom[NetStartupInfoBegin] = XInternAtom(dpy, "_NET_STARTUP_INFO_BEGIN", False);
	...
	dc.urg[ColBG] = getcolor(selbgcolor, dc.xft.sel + ColBG);
	dc.urg[ColFG] = getcolor(selfgcolor, dc.xft.sel + ColFG);
...
textnw(const char *text, unsigned int len)
{
	...
	if (!parsed)
	{
		pango_layout_set_text(dc.font.layout, "invalid", -1);
	}
	else
	{
		pango_layout_set_attributes(dc.font.layout, attrList);
...
updatecurrenddesktop()
{
	long data[] = { selmon->tagset[selmon->seltags] - 1 };
...
updatesystray(void)
{
	...
	if (!systray)
	{
		...
		XStoreName(dpy, systray->win, appname);
		classHint = XAllocClassHint();
		if (classHint)
		{
			classHint->res_name = appname;
			classHint->res_class = "Systray";
		}
		XSetClassHint(dpy, systray->win, classHint);
		XFree(classHint);
...
updatewmhints(Client *c)
{
	...
	if ((wmh = XGetWMHints(dpy, c->win)))
	{
		...
		else
		{
			...
			if (c->isurgent)
				XSetWindowBorder(dpy, c->win, dc.urg[ColUrgBorder]);
		}
